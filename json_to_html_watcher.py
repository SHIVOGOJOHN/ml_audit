        ...
        if (auditData.operations && Array.isArray(auditData.operations)) {
            auditData.operations.forEach((op, index) => {
                ...
                `;
                    ...
                    detailsHtml = `
                        <div class="property-row"><span class="prop-key">Column:</span><span class="prop-value">${op.column}</span></div>
...
def process_file(json_path, output_dir):
    ...
    try:
        ...
        if 'operations' not in data:
            print(f"Skipping {json_path}: 'operations' key missing.")
            return
        ...
        # Get just the filename and create HTML path in output directory
...
def main():
    print("Starting JSON to HTML watcher...")
    ...
    audit_trails_dir = os.path.join(script_dir, 'audit_trails')
    visualizations_dir = os.path.join(script_dir, 'visualizations')
    
    # Create directories if they don't exist
    os.makedirs(audit_trails_dir, exist_ok=True)
    os.makedirs(visualizations_dir, exist_ok=True)
    
    print(f"Monitoring: {audit_trails_dir}")
    print(f"Output to: {visualizations_dir}")
    
    while True:
        json_files = glob.glob(os.path.join(audit_trails_dir, '*.json'))
        
        for json_file in json_files:
            json_filename = os.path.basename(json_file)
            html_filename = json_filename.replace('.json', '.html')
            html_file = os.path.join(visualizations_dir, html_filename)
            ...
            if not os.path.exists(html_file):
                ...
                # Check if JSON is newer than HTML
            ...
            if should_process:
                process_file(json_file, visualizations_dir)
